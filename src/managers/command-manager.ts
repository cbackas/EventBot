...
export class CommandManager {
  ...
  constructor (private readonly token: string, private readonly clientId: string, private readonly guildId: string) { }
  ...
  public registerCommands = async (): Promise<void> => {
    ...
    try {
      ...
      const registeredCommands: string[] = this.commands.map((cmd) => cmd.slashCommand.main.name)
      console.log('Slash commands registered:\n' + (registeredCommands.length > 0 ? registeredCommands.join('\n') : 'None!'))
    } catch (error) {
      console.error(error)
    }
  }
